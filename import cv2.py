...
def main(video_path, output_dir):
    ...
    for frame_file in sorted(os.listdir(output_dir)):
        ...
        cv2.putText(road_segmented, 'Road and Sidewalk Segmentation', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        cv2.imshow("Road and Sidewalk Segmentation", road_segmented)

        cv2.putText(frame, 'Original Frame', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        cv2.imshow("Original Frame", frame)

        crosswalk = detect_crosswalk(frame)
        cv2.putText(crosswalk, 'Crosswalk Detection', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
